...
// ForgeRouter is a high-performance HTTP router with OpenAPI support
...
type OpinionatedHandler[TInput any, TOutput any] func(ctx *ForgeContext, input TInput) (*TOutput, error)
...
// NewRouter creates a new ForgeRouter instance
func NewRouter() *ForgeRouter {
	return &ForgeRouter{
		...
		options: RouterOptions{
			...
			HandleOPTIONS:          true,
			OpenAPITitle:           "ForgeRouter API",
			OpenAPIVersion:         "1.0.0",
			OpenAPIDescription:     "API documentation generated by ForgeRouter",
		},
		openAPISpec: &OpenAPISpec{
			OpenAPI: "3.1.1",
			Info: OpenAPIInfo{
				Title:       "ForgeRouter API",
				Version:     "1.0.0",
				Description: "API documentation generated by ForgeRouter",
			},
...
// Ensure ForgeRouter implements Router interface
var _ Router = (*ForgeRouter)(nil)

// GET Standard HTTP method handlers
func (r *ForgeRouter) GET(pattern string, handler HandlerFunc) {
	r.addRoute("GET", pattern, handler)
}

func (r *ForgeRouter) POST(pattern string, handler HandlerFunc) {
	r.addRoute("POST", pattern, handler)
}

func (r *ForgeRouter) PUT(pattern string, handler HandlerFunc) {
	r.addRoute("PUT", pattern, handler)
}

func (r *ForgeRouter) DELETE(pattern string, handler HandlerFunc) {
	r.addRoute("DELETE", pattern, handler)
}

func (r *ForgeRouter) PATCH(pattern string, handler HandlerFunc) {
	r.addRoute("PATCH", pattern, handler)
}

func (r *ForgeRouter) HEAD(pattern string, handler HandlerFunc) {
	r.addRoute("HEAD", pattern, handler)
}

func (r *ForgeRouter) OPTIONS(pattern string, handler HandlerFunc) {
	r.addRoute("OPTIONS", pattern, handler)
}

func (r *ForgeRouter) Handle(method, pattern string, handler HandlerFunc) {
	r.addRoute(method, pattern, handler)
}

func (r *ForgeRouter) HandleFunc(method, pattern string, handler http.HandlerFunc) {
...
func (r *ForgeRouter) OpinionatedGET(pattern string, handler interface{}, opts ...HandlerOption) {
	r.registerOpinionatedHandlerWithMiddleware("GET", pattern, handler, opts...)
}

func (r *ForgeRouter) OpinionatedPOST(pattern string, handler interface{}, opts ...HandlerOption) {
	r.registerOpinionatedHandlerWithMiddleware("POST", pattern, handler, opts...)
}

func (r *ForgeRouter) OpinionatedPUT(pattern string, handler interface{}, opts ...HandlerOption) {
	r.registerOpinionatedHandlerWithMiddleware("PUT", pattern, handler, opts...)
}

func (r *ForgeRouter) OpinionatedDELETE(pattern string, handler interface{}, opts ...HandlerOption) {
	r.registerOpinionatedHandlerWithMiddleware("DELETE", pattern, handler, opts...)
}

func (r *ForgeRouter) OpinionatedPATCH(pattern string, handler interface{}, opts ...HandlerOption) {
	r.registerOpinionatedHandlerWithMiddleware("PATCH", pattern, handler, opts...)
}

func (r *ForgeRouter) registerOpinionatedHandlerWithMiddleware(method, pattern string, handler interface{}, opts ...HandlerOption) {
	...
	if handlerType.NumIn() != 2 || handlerType.NumOut() != 2 {
		panic("handler must have signature func(*ForgeContext, InputType) (*OutputType, error)")
	}
...
func (r *ForgeRouter) generateOpenAPIForHandlerWithMiddleware(info *HandlerInfo) {
...
func (r *ForgeRouter) createOpinionatedWrapperWithMiddleware(handler interface{}, inputType, outputType reflect.Type, handlerInfo *HandlerInfo) HandlerFunc {
...
func (r *ForgeRouter) registerOpinionatedHandler(method, pattern string, handler interface{}, opts ...HandlerOption) {
	handlerType := reflect.TypeOf(handler)
	if handlerType.Kind() != reflect.Func {
		panic("handler must be a function")
	}
...
func (r *ForgeRouter) createOpinionatedWrapper(handler interface{}, inputType, outputType reflect.Type) HandlerFunc {
	handlerValue := reflect.ValueOf(handler)
	...
	return func(w http.ResponseWriter, req *http.Request) {
		...
		// Create ForgeContext with enhanced error handling
		ctx := &ForgeContext{
			Request:  req,
			Response: w,
			router:   r,
...
func (r *ForgeRouter) handleError(w http.ResponseWriter, req *http.Request, err error) {
...
func (r *ForgeRouter) writeErrorResponse(w http.ResponseWriter, req *http.Request, apiErr APIError) {
	w.Header().Set("Content-Type", "application/json")
...
func (r *ForgeRouter) bindParameters(ctx *ForgeContext, input interface{}) error {
...
func (r *ForgeRouter) setFieldValue(field reflect.Value, value string) error {
	if value == "" {
		return nil
	}
...
func (r *ForgeRouter) generateOpenAPIForHandler(info *HandlerInfo) {
...
func (r *ForgeRouter) convertToOpenAPIPath(path string) string {
...
func (r *ForgeRouter) addStandardErrorResponses(operation *OpenAPIOperation, method string) {
...
func (r *ForgeRouter) ensureErrorSchemasRegistered() {
...
func (r *ForgeRouter) typeToSchema(t reflect.Type) OpenAPISchema {
...
func (r *ForgeRouter) generateStructSchema(t reflect.Type) OpenAPISchema {
	schema := OpenAPISchema{
		Type:       "object",
...
func (r *ForgeRouter) EnableOpenAPI() {
	// Serve OpenAPI spec
...
func (r *ForgeRouter) Use(middleware ...MiddlewareFunc) {
	r.middleware = append(r.middleware, middleware...)
}

// Group creates a new route group with empty prefix
func (r *ForgeRouter) Group() Router {
	return &RouteGroup{
		router:     r,
		prefix:     "",
...
func (r *ForgeRouter) GroupFunc(fn func(r Router)) Router {
	group := &RouteGroup{
		router:     r,
		prefix:     "",
		middleware: make([]MiddlewareFunc, 0),
...
func (r *ForgeRouter) Route(pattern string, fn func(r Router)) Router {
	group := &RouteGroup{
		router: r,
		prefix: pattern,
	}
...
func (r *ForgeRouter) findFixedPath(method, path string, params *Params) (string, bool) {
...
func (r *ForgeRouter) pathExistsForOtherMethods(path, currentMethod string) bool {
...
func (r *ForgeRouter) getAllowedMethods(path string) []string {
	var methods []string
	for method := range r.trees {
		params := r.pool.Get().(*Params)
		params.Reset()
...
func (r *ForgeRouter) findHandler(method, path string, params *Params) HandlerFunc {
	root := r.trees[method]
	if root == nil {
		return nil
	}
...
func (r *ForgeRouter) addRoute(method, path string, handler HandlerFunc) {
	if path == "" {
...
func (r *ForgeRouter) SetTrailingSlashRedirect(enabled bool) {
	r.options.RedirectTrailingSlash = enabled
}

func (r *ForgeRouter) SetFixedPathRedirect(enabled bool) {
	r.options.RedirectFixedPath = enabled
}

func (r *ForgeRouter) SetMethodNotAllowedHandler(handler http.Handler) {
	r.methodNotAllowed = handler
}

func (r *ForgeRouter) SetNotFoundHandler(handler http.Handler) {
	r.notFoundHandler = handler
}

func (r *ForgeRouter) extractURLParams(path, method string, params *Params) {
...
func (r *ForgeRouter) matchAndExtractParams(n *node, path string, params *Params) bool {
	if len(path) == 0 {
