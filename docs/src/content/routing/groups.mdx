...
Route groups in ForgeRouter provide a powerful way to organize related routes and apply middleware to multiple routes at once. Groups support nested hierarchies, middleware inheritance, and clean URL patterns.

## Basic Route Groups

### Creating Groups

ForgeRouter provides two methods for creating route groups:
...
```go
func setupAPI(router *forgerouter.FastRouter) {
    // Global middleware
...
```go
func setupMultiTenantAPI(router *forgerouter.FastRouter) {
    router.Use(forgerouter.Logger)
    router.Use(forgerouter.Recoverer)
...
```go
func setupGateway(router *forgerouter.FastRouter) {
    router.Use(forgerouter.Logger)
    router.Use(forgerouter.Recoverer)
    router.Use(corsMiddleware())
...
router.Route("/api/v1", func(api router.Router) {
    api.Route("/users", func(users router.Router) {
        ...
        users.OpinionatedGET("/", func(ctx *forgerouter.FastContext, req ListUsersRequest) (*ListUsersResponse, error) {
            // Implementation
            return &ListUsersResponse{}, nil
        }, forgerouter.WithSummary("List Users"), forgerouter.WithTags("users"))

        users.OpinionatedPOST("/", func(ctx *forgerouter.FastContext, req CreateUserRequest) (*CreateUserResponse, error) {
            // Implementation
            return &CreateUserResponse{}, nil
        }, forgerouter.WithSummary("Create User"), forgerouter.WithTags("users"))

        users.OpinionatedGET("/:id", func(ctx *forgerouter.FastContext, req GetUserRequest) (*GetUserResponse, error) {
            // Implementation
            return &GetUserResponse{}, nil
        }, forgerouter.WithSummary("Get User"), forgerouter.WithTags("users"))
    })
})
```
...
Route groups in ForgeRouter provide a clean, hierarchical way to organize your API while maintaining middleware inheritance and generating comprehensive OpenAPI documentation.