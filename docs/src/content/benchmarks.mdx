...
ForgeRouter is designed for high performance while maintaining rich features like automatic OpenAPI generation. This guide covers performance benchmarks, comparisons with other routers, and optimization strategies.

## Quick Performance Overview

Based on comprehensive benchmarks on Apple M3 Max (ARM64), ForgeRouter demonstrates competitive performance:

| Router | ns/op | B/op | allocs/op | Relative Speed |
|--------|-------|------|-----------|----------------|
| **Gin** | 87.6 | 48 | 1 | 1.00x (fastest) |
| **FastRouter** | 208.5 | 386 | 4 | 2.38x |
| **Chi** | 210.7 | 370 | 3 | 2.41x |

import { Callout } from 'nextra/components'

<Callout type="info">
    **Performance Note**: While ForgeRouter is 2.4x slower than Gin in synthetic benchmarks, it provides significant additional features like automatic OpenAPI generation, typed handlers, and advanced middleware that Gin lacks.
</Callout>
...
- **ForgeRouter**: 100-200 ns/op with efficient parameter handling
...
- **ForgeRouter advantage**: Opinionated middleware optimizations
...
```bash
BenchmarkParameterRoutes/FastRouter_/users/123-8    5000000    208.5 ns/op    386 B/op    4 allocs/op
```

**Breaking down the result:**
- `BenchmarkParameterRoutes`: Test category
- `FastRouter_/users/123`: Specific test case
...
### When to Choose ForgeRouter

**Choose ForgeRouter when you need:**
...
func (r *FastRouter) findRoute(path string) Handler {
...
func GetUser(ctx *forgerouter.FastContext, req GetUserRequest) (*GetUserResponse, error) {
    ...
    if err != nil {
        return nil, forgerouter.NotFound("User")
    }
...
ForgeRouter strikes a balance between performance and features:
...
For most applications, ForgeRouter's feature set outweighs the minor performance overhead, especially when considering the time saved in development and maintenance.